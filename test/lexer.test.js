...
test("Lexer Functionality", async (t) => {
    ...
    await t.test("should tokenize operators and delimiters", () => {
        ...
        const expected_tokens = [
            { type: "Assign" }, { type: "Plus" }, { type: "LParen" }, { type: "RParen" },
            { type: "LBrace" }, { type: "RBrace" }, { type: "Comma" }, { type: "Semicolon" },
            { type: "Eof" }
        ];
        const tokens = wasm._test_lexer(input);
        assert.deepStrictEqual(tokens.map(t => ({type: t.type})), expected_tokens, "Failed to tokenize basic operators");
    });
    ...
    await t.test("should tokenize a simple function declaration", () => {
        ...
        `;
        const expected_tokens = [
            "Function", "Ident", "Ident", "LParen", "Ident", "Ident", "Comma", "Ident", "Ident", "RParen", "LBrace",
            "Return", "Ident", "Plus", "Ident", "Semicolon", "RBrace", "Eof"
        ];
        const tokens = wasm._test_lexer(input);
        assert.deepStrictEqual(tokens.map(t => t.type), expected_tokens, "Failed to tokenize function declaration");
    });
    ...
    await t.test("should tokenize logical and comparison operators", () => {
        ...
        const expected_tokens = [
            "Ident", "Eq", "Ident", "StrictEq", "Ident",
            "LogicalAnd", "Ident", "LogicalOr", "Ident",
            "And", "Ident", "Or", "Ident", "Eof"
        ];
        const tokens = wasm._test_lexer(input);
        assert.deepStrictEqual(tokens.map(t => t.type), expected_tokens, "Failed to tokenize logical and comparison ops");
    });
